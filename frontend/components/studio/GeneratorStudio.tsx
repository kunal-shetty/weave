  ...
  return (
    <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          {(['wireframe', 'code', 'prompt'] as InputMode[]).map((mode) => (
            <Button
              ...
              className={`btn-3d gap-1 sm:gap-2 backdrop-blur-sm border shadow-lg text-xs sm:text-sm ${
                activeModes.has(mode)
                  ? 'bg-primary text-primary-foreground border-primary'
                  : 'bg-secondary/70 text-foreground hover:bg-secondary/50 border-border/30'
              }`}
            >
              {mode === 'wireframe' && <ImageIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
              {mode === 'code' && <Code2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
              {mode === 'prompt' && <FileText className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <Popover open={configOpen} onOpenChange={setConfigOpen}>
            <PopoverTrigger asChild>
              <Button
                className="btn-3d gap-1 sm:gap-2 bg-secondary/70 text-foreground hover:bg-secondary/50 border border-border/30 shadow-lg text-xs sm:text-sm"
              >
          ...
          {lastJob && (
            <>
              <Link href={`/preview/${lastJob.pageName}`}>
                <Button className="btn-3d btn-glow gap-1 sm:gap-2 bg-secondary/70 text-foreground hover:bg-secondary/50 border border-border/30 shadow-lg text-xs sm:text-sm">
                  <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> {!isMobile && 'Preview'}
                </Button>
              </Link>
              <Button onClick={handleDownloadZip} className="btn-3d gap-1 sm:gap-2 bg-secondary/70 text-foreground hover:bg-secondary/50 border border-border/30 shadow-lg text-xs sm:text-sm">
                <Download className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> {!isMobile && 'Export'}
              </Button>
            </>
          )}
        </div>
      ...
      <div className="relative z-10 flex-1 flex overflow-hidden">
        <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 pb-4 sm:pb-6 overflow-y-auto">
          {/* Orb + Title */}
          {!lastJob && (
            <div className="flex flex-col items-center mb-6 sm:mb-8">
              ...
              <h1 className="text-2xl sm:text-4xl font-semibold text-foreground text-center font-[var(--font-heading)] tracking-tight px-2">
                {isMobile ? 'Generate UI Section' : 'Generate a UI Section'}
              </h1>
              <p className="text-muted-foreground mt-2 text-center text-sm sm:text-base px-2">
                {isMobile ? 'Upload, paste, or describe — or combine all three.' : 'Upload a wireframe, paste code, write a prompt — or combine all three.'}
              </p>
            </div>
          )}

          {/* Input Panels */}
          <div className="w-full max-w-4xl space-y-3 sm:space-y-4 px-2 sm:px-0">
            {/* Error */}
            {error && (
              <div className="flex items-center gap-3 p-3 sm:p-4 bg-destructive/10 border border-destructive/30 rounded-xl text-destructive text-xs sm:text-sm">
            ...
            {activeModes.has('wireframe') && (
              <div
                className="input-3d bg-gradient-to-br from-secondary/70 via-secondary/60 to-secondary/50 backdrop-blur-xl rounded-2xl border border-border/50 p-3 sm:p-4 shadow-2xl"
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs sm:text-sm font-medium text-foreground">
                    ...
                    {wireframeFile ? (
                      <span className="truncate max-w-[200px] sm:max-w-[280px]">{wireframeFile.name}</span>
                    ) : (
                      'Wireframe'
                    )}
                  </div>
                ...
                {!wireframeFile && (
                  <div
                    className="mt-3 border-2 border-dashed border-border/50 rounded-xl p-4 sm:p-6 text-center cursor-pointer hover:border-border transition-colors"
                    onClick={() => fileInputRef.current?.click()}
                  >
                    <Upload className="w-5 h-5 sm:w-6 sm:h-6 mx-auto mb-2 text-muted-foreground" />
                    <p className="text-xs sm:text-sm text-muted-foreground">Drop PNG/JPG/WebP or click to upload</p>
            ...
            {activeModes.has('code') && (
              <div className="input-3d bg-gradient-to-br from-secondary/70 via-secondary/60 to-secondary/50 backdrop-blur-xl rounded-2xl border border-border/50 p-3 sm:p-4 shadow-2xl">
                <div className="flex items-center gap-2 mb-3 text-xs sm:text-sm font-medium text-foreground">
                  <Code2 className="w-4 h-4" /> Existing Code Patterns
                </div>
                <textarea
                  className="w-full bg-transparent border-none outline-none resize-none text-foreground placeholder:text-muted-foreground text-xs sm:text-sm font-mono min-h-[120px] sm:min-h-[150px]"
            ...
            <div className="input-3d bg-gradient-to-br from-secondary/70 via-secondary/60 to-secondary/50 backdrop-blur-xl rounded-2xl border border-border/50 p-3 sm:p-4 shadow-2xl">
              {activeModes.has('prompt') && (
                <div className="flex items-start gap-2 sm:gap-3 mb-3 sm:mb-4">
                  ...
                  <textarea
                    className="flex-1 bg-transparent border-none outline-none resize-none text-foreground placeholder:text-muted-foreground text-base sm:text-lg min-h-[60px] sm:min-h-[80px]"
                    placeholder={isMobile ? "Describe your section..." : "Describe your section… e.g. 'Split hero layout: athlete image left, bold headline, 3 stat cards, red CTA button. Fitness brand.'"}
                    value={prompt}
              ...
              <div className="flex items-center justify-between pt-2 border-t border-border/30">
                ...
                <Button
                  ...
                  disabled={generating}
                  className="btn-3d btn-glow gap-1.5 sm:gap-2 bg-gradient-to-br from-primary via-gray-900 to-black hover:from-gray-900 hover:to-black text-white shadow-xl text-xs sm:text-sm"
                >
                  {generating ? (
                    <><span className="animate-spin">◌</span> {isMobile ? 'Gen…' : progress || 'Generating…'}</>
                  ) : (
                    <><Wand2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Generate</>
                  )}
                </Button>
              </div>
            </div>
            ...
            {lastJob && showJsx && (
              <div className="input-3d bg-gradient-to-br from-secondary/70 via-secondary/60 to-secondary/50 backdrop-blur-xl rounded-2xl border border-border/50 p-3 sm:p-4 shadow-2xl">
                ...
                <pre className="text-xs text-muted-foreground overflow-x-auto max-h-48 sm:max-h-64 font-mono whitespace-pre-wrap">
                  {lastJob.generatedJsx}
                </pre>
                <div className="mt-3 flex gap-2">
                  <Link href={`/preview/${lastJob.pageName}`}>
                    <Button className="btn-3d btn-glow gap-2 bg-gradient-to-br from-primary via-gray-900 to-black text-white text-xs sm:text-sm">
                      <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Open Preview
                    </Button>
                  </Link>
                </div>
              </div>
        ...
        {/* Job History Sidebar - hidden on mobile */}
        {jobHistory.length > 0 && !isMobile && <JobHistoryPanel />}
      </div>
    </main>
  );
}
